...
D_solver = optim.Adam(D_A.parameters(), lr=lr)
...
for it in range(0,1000):
	...
	for i in range(data_nbr/mb_size):
		...
		# Sample data
			...
			l_d = l_d_A_real + l_d_A_fake1/2 + l_d_A_fake2/2
			l_d.backward()
		...
		l_gan_A = bce(D_A(x_fakeAB), real_tensor) / 2
		l_gan_B = bce(D_A(x_fakeBA), real_tensor) / 2
